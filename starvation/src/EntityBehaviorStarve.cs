...
namespace Starvation
{
    public class EntityBehaviorStarve : EntityBehavior
    {
        ...
        private double timeSpeedMult = 1.0;
        private double currentBMR = 6000;
        private float gameHungerSpeed = 1.0f; // default = 1
        private double dayToGameQuarter = DAY_TO_GAMEQUARTERSECOND;
        private double energyGainPerQuarterSecond = 0.0; //gain from digestation or metabolism to be applied at the fast ticking callback
        private double heatIndexTemp = 15; // heat index temperature at entity's current location
        private double metabolismFactor = 1.0; // used to change energy usage depending on energy reserves
        private double energyLevelPenalty = 1.0;
        private double maxGastrointestinalReserves = MAX_GASTROINTESTINAL_RESERVES * 1500.0;
        private bool playerBlackedOut = false;

        private EnergyReserveLevel energyReserveLevel = EnergyReserveLevel.MEDIUM;
        ...
        public double gastrointestinalReserves
        {
            get => entity.WatchedAttributes.GetDouble(
                    ...
                    INITIAL_GASTROINTESTINAL_RESERVES
                );
            set
            {
        ...
        private void _ServerTickSlow(float _)
        {
            ...
            heatIndexTemp = ModSystemStarvation.HeatIndexTemperature(
            ...
            bodyWeight =
                ModSystemStarvation.EnergyReservesToBMI(energyReserves)
                * Math.Pow(entity.Properties.EyeHeight, 2);
            ...
            if (
                bodyWeight < STARVE_THRESHOLD_EXTREME
                || energyReserveLevel <= EnergyReserveLevel.ZERO
            )
            {
                entity.WatchedAttributes.SetFloat(
                    "intoxication",
        ...
        private void _ServerTick250(float deltaTime)
        {
            ...
            if (timeSpeedMult > 5.0) // expects night speed mult beeing larger 5.0
            {
                ...
                energyReserves -=
                    ((kJPerGameQuarterSecond * metabolismFactor) - energyGainPerQuarterSecond)
                    * timeSpeedMult; // reserves - steady usage + energy gain from digestation or metabolism (which is calculated on 5s base while this is applied 20 times as often->so /20.)
            }
            else //depend on the calculation in the slow ticking callback for better performance
            {
                energyReserves -=
                    (kJPerGameQuarterSecond * metabolismFactor) - energyGainPerQuarterSecond; // reserves - steady usage + energy gain from digestation or metabolism (which is calculated on 5s base while this is applied 20 times as often->so /20.)
            }

            if (energyReserves < 0.0)
                energyReserves = 0.0; //prevent underflow
        }
        ...
        public bool IsSelf =>
            entity?.WatchedAttributes.GetString("playerUID")
            == ModSystemStarvation.clientAPI?.Settings.String["playeruid"];
        ...
        {
            if (energyReserves > 0)
                return 1; //no negative effects when satiated
            return bodyWeight switch
            {
        ...
        public float MoveSpeedPenalty()
        {
            ...
            if (
                bhunger.SaturationLossDelayDairy > 0
                || bhunger.SaturationLossDelayFruit > 0
                || bhunger.SaturationLossDelayGrain > 0
                || bhunger.SaturationLossDelayProtein > 0
                || bhunger.SaturationLossDelayVegetable > 0
            )
            {
        ...
        public static HungerLevel WeightToHungerLevel(double weight)
        {
            return weight switch
            {
                ...
                <= STARVE_THRESHOLD_SEVERE
                and > 0.5 * (STARVE_THRESHOLD_SEVERE + STARVE_THRESHOLD_EXTREME)
                    => HungerLevel.Severe, // severe
                <= 0.5 * (STARVE_THRESHOLD_SEVERE + STARVE_THRESHOLD_EXTREME)
                and > STARVE_THRESHOLD_EXTREME
